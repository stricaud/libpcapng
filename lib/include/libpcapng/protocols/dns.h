...
size_t libpcapng_dns_build_question(uint8_t *buf, const char *qname, uint16_t qtype, uint16_t qclass);
size_t libpcapng_dns_build_answer_a(uint8_t *buf, const char *name, uint32_t ipv4_be, uint32_t ttl);
